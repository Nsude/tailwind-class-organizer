...
function getCategory(className: string): string {
    ...
    if (prefixMap[cleanClass]) return prefixMap[cleanClass];
    ...
    if (className.startsWith('text-')) {
        ...
        if (/^text-(xs|sm|base|lg|xl|\d+xl)/.test(className)) return "Typography";
        return "Colors";
    }
...
export function sortClasses(classString: string): string {
    const classes = classString.split(/\s+/).filter(c => c.length > 0);
    
    const grouped: { [key: string]: string[] } = {};
    sortOrder.forEach(g => grouped[g] = []);
